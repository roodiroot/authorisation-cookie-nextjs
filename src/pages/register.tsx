          ...
          <div className="relative z-10 flex flex-1 flex-col bg-white px-4 py-10 shadow-2xl sm:justify-center md:flex-none md:px-28">
              <div className="mx-auto w-full max-w-md sm:px-4 md:w-96 md:max-w-sm md:px-0">
                  <div className="flex flex-col">
                      ...
                      <div className="mt-20">
                          <h2 className="text-lg font-semibold text-gray-900">Регистрация на сайте</h2>
                          <p className="mt-2 text-sm text-gray-700">У вас уже есть акаунт?   
                              <Link className="font-medium text-blue-600 hover:underline" href="/login"> Войти </Link>
                          </p>
                      </div>
                  </div>
                  <form action="#" className="mt-10 grid grid-cols-1 gap-y-8">
                        <div className="">
                            <label className="mb-3 block text-sm font-medium text-gray-700">Ваше имя</label>
                            <input required onChange={e => setName(e.target.value)} value={name} type="email" name="email" autoComplete="email" className="block w-full appearance-none rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-blue-500 sm:text-sm"/>
                        </div>
                        <div className="">
                            <label className="mb-3 block text-sm font-medium text-gray-700">Email адрес</label>
                            <input required onChange={e => setEmail(e.target.value)} value={email} type="email" name="email" autoComplete="email" className="block w-full appearance-none rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-blue-500 sm:text-sm"/>
                        </div>
                        <div className="">
                            <label className="mb-3 block text-sm font-medium text-gray-700">Пароль</label>
                            <input required onChange={e => setPassword(e.target.value)} value={password} type="password" name="password" autoComplete="current-password"  className="block w-full appearance-none rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-blue-500 sm:text-sm"/>
                        </div>
                        <div>
                            <button onClick={submit} className="group inline-flex items-center justify-center rounded-full py-2 px-4 text-sm font-semibold focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 bg-blue-600 text-white hover:text-slate-100 hover:bg-blue-500 active:bg-blue-800 active:text-blue-100 focus-visible:outline-blue-600 w-full" type="submit">
                                <span>Зарегестрироваться <span aria-hidden="true">→</span></span>
                            </button>
                        </div>
                    </form>
              </div>
